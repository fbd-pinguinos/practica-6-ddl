...
CREATE TABLE cliente(
	...
	horaEntrada TIME, 
	horaSalida TIME,
	ocupacion VARCHAR(10) NOT NULL CHECK(ocupacion <> '')
);
...
COMMENT ON COLUMN cliente.cp IS 'Código postal donde vive la persona';
COMMENT ON COLUMN cliente.numeroCalle IS 'Número de la calle donde vive la persona';
...
ON UPDATE CASCADE ON DELETE CASCADE;
ALTER TABLE manejar ADD CONSTRAINT manejar_fkey2 FOREIGN KEY(nEconomico) REFERENCES vehiculoFuncional
ON UPDATE CASCADE ON DELETE CASCADE;
